...
document.addEventListener('DOMContentLoaded', function () {
	...
	games.forEach(function (game) {
		...
		function updateClock() {
			...
			timeEl.textContent = formatTime(parisNow);
			dateEl.textContent = formatDate(parisNow);
			zoneEl.textContent = 'Paris Time';
		}
...
if (!function_exists('zo_game_paris_clock_render')) {
	function zo_game_paris_clock_render($post_id = 0, $module = array()) {
		...
		ob_start();
		?>
		<div class="zo-game-root zo-game-root--paris-clock" id="<?php echo esc_attr($instance_id); ?>">
			<h2 class="zo-pc-title">Paris Clock</h2>
			...
			<div class="zo-pc-zone">Paris Time</div>

			<div class="zo-pc-actions">
				<button type="button" class="zo-pc-button zo-pc-button--format">12/24 Format</button>
				<button type="button" class="zo-pc-button zo-pc-button--refresh">Refresh</button>
			</div>
		</div>
		<?php
		return ob_get_clean();
