...
function monsters(){var a=[],count=Math.min(2+Math.floor(state.level/3),15);for(var n=0;n<count;n++){var x,y;do{x=2+Math.floor(Math.random()*(S-4));y=2+Math.floor(Math.random()*(S-4))}while(state.map[y][x]!==0||(x<4&&y<4));a.push({x:x+.5,y:y+.5,hp:30+state.level*10,max:30+state.level*10,speed:.0025+state.level*.00004,hit:0})}return a}
function reset(){state={level:1,map:maze(),x:1.5,y:1.5,a:0,hp:100,enemies:[],cool:0,run:false,over:false,won:false};state.enemies=monsters();message.textContent='Başlat düğmesine bas. Yeşil çıkışa ulaşarak bir sonraki seviyeye geç.';updateHud();draw()}
...
function zo_monster_maze_render($post_id = 0, $module = array()) {
	...
	<div class="zo-game-root zo-game-root--monster-maze">
		<div class="zo-mm-card">
			...
			<p class="zo-mm-help"><strong>Kontroller:</strong> Oklar/WASD ile hareket et, sol-sağ oklarla dön, SPACE veya Ateş ile vur. Bir sonraki seviyeye geçmek için yeşil çıkışa ulaş; canavarları yenmek zorunlu değil.</p>
		</div>
	</div>
