...
document.addEventListener('DOMContentLoaded', function () {
	...
	games.forEach(function (game, gameIndex) {
		...
		const apiProviderSelect = game.querySelector('.zo-asp-api-provider');
		...
		function loadShareStateFromHash() {
			...
			if (typeof data.provider === 'string') apiProviderSelect.value = data.provider;
			if (typeof data.apiUrl === 'string') apiUrlInput.value = data.apiUrl;
			if (typeof data.model === 'string') apiModelInput.value = data.model;
			...
			updateApiProviderDefaults(false);
			updateApiVisibility();
			updateStats();
			return true;
		}
		...
		function updateApiProviderDefaults(forceOverride) {
			const provider = apiProviderSelect.value;
			const shouldSetUrl = forceOverride || !apiUrlInput.value.trim();
			const shouldSetModel = forceOverride || !apiModelInput.value.trim();

			if (provider === 'gemini') {
				if (shouldSetUrl) {
					apiUrlInput.value = 'https://generativelanguage.googleapis.com/v1beta/images:generate';
				}
				if (shouldSetModel) {
					apiModelInput.value = 'gemini-image';
				}
			} else if (provider === 'huggingface') {
				if (shouldSetUrl) {
					apiUrlInput.value = 'https://api-inference.huggingface.co/models/stabilityai/stable-diffusion-xl-base-1.0';
				}
				if (shouldSetModel) {
					apiModelInput.value = '';
				}
				...
				if (shouldSetUrl) {
					apiUrlInput.value = 'https://fal.run/fal-ai/flux/dev';
				}
				if (shouldSetModel) {
					apiModelInput.value = 'flux';
				}
			} else if (provider === 'sd') {
				if (shouldSetUrl) {
					apiUrlInput.value = 'https://api.stability.ai/v2beta/stable-image/generate/core';
				}
				if (shouldSetModel) {
					apiModelInput.value = 'sdxl';
				}
			} else if (provider === 'selfhosted') {
				if (shouldSetUrl) {
					apiUrlInput.value = 'https://your-domain.example.com/api/generate-image';
				}
				if (shouldSetModel) {
					apiModelInput.value = 'local-model';
				}
			}
		}
		...
		function saveCurrentAsJpg() {
			...
			img.onerror = function () {
				setStatus(statusEl, 'JPG export failed. PNG should still work.', 'bad');
			};
		...
		async function apiRenderFlow() {
			...
			try {
				let response;
				let imageSrc = '';
				const sizeValue = sizeSelect.value === 'landscape'
					? '1536x1024'
					: (sizeSelect.value === 'portrait' ? '1024x1536' : '1024x1024');
				...
				if (provider === 'gemini') {
					...
					if (json && json.images && json.images[0] && json.images[0].data) {
						imageSrc = 'data:image/png;base64,' + json.images[0].data;
					} else if (json && json.data && json.data[0] && json.data[0].b64_json) {
						imageSrc = 'data:image/png;base64,' + json.data[0].b64_json;
					} else if (json && json.data && json.data[0] && json.data[0].url) {
					...
					if (contentType.indexOf('application/json') !== -1) {
						const json = await response.json();
						if (json && json.image) {
							imageSrc = 'data:image/png;base64,' + json.image;
						} else if (json && json.data && json.data[0] && json.data[0].b64_json) {
							imageSrc = 'data:image/png;base64,' + json.data[0].b64_json;
						} else if (json && json.data && json.data[0] && json.data[0].url) {
							imageSrc = json.data[0].url;
					...
					if (json && json.data && json.data[0] && json.data[0].b64_json) {
						...
						imageSrc = json.data[0].url;
					} else if (json && json.image_url) {
						imageSrc = json.image_url;
					} else if (json && json.b64_json) {
		...
		function handleChat() {
			...
			if (containsAny(low, ['hello', 'hi', 'merhaba', 'selam'])) {
				reply = lang === 'tr'
					? 'Merhaba. Türkçe-İngilizce prompt çeviririm, kid-safe kontrolü yaparım, coin sistemi var, share link yaparım ve 5 API sağlayıcısı desteklerim.'
					: 'Hello. I can translate Turkish-English prompts, do kid-safe checks, track coins, make share links, and use 5 API providers.';
				...
				reply = lang === 'tr'
					? 'API mode açıkken Gemini, HuggingFace, fal.ai, SD ve self-hosted sağlayıcıları seçebilirsin.'
					: 'With API mode on, you can choose Gemini, HuggingFace, fal.ai, SD, or self-hosted providers.';
			} else {
				reply = lang === 'tr'
		...
		function resetAll() {
			...
			chatLog.innerHTML = '';
			updateApiProviderDefaults(true);
			updateApiVisibility();
			updateStats();
			resetPreviewMode();
		...
		btnApiDemo.addEventListener('click', function () {
			...
			updateApiProviderDefaults(true);
			updateApiVisibility();
			setStatus(statusEl, 'API demo settings loaded. Add your key and render.');
		});
		...
		<div class="zo-game-root zo-game-root--ai-studio-pro" id="<?php echo esc_attr($instance_id); ?>">
			<div class="zo-asp-shell">
				<div class="zo-asp-topbar">
					...
					<p class="zo-asp-subtitle">English + Türkçe chatbot, optional real AI image API mode, automatic Turkish-English prompt translation, coins system, kid-safe mode, multiplayer-style share links, PNG/JPG export, upload support, and local fallback art rendering.</p>
				</div>
						...
						<div class="zo-asp-card">
							...
							<div class="zo-asp-stack">
								...
								<div>
									...
									<input type="file" accept="image/*" class="zo-asp-file" />
								</div>
								...
								<div class="zo-asp-card zo-asp-api-fields zo-asp-hidden" style="margin:0;">
									<div class="zo-asp-stack">
										<div>
											<label class="zo-asp-label">API provider</label>
											<select class="zo-asp-select zo-asp-api-provider">
												<option value="gemini">Google Gemini</option>
												<option value="huggingface">HuggingFace</option>
												<option value="fal">fal.ai</option>
												<option value="sd">Stable Diffusion / FLUX</option>
												<option value="selfhosted">Self-hosted / Custom</option>
											</select>
										</div>

										<div>
											<label class="zo-asp-label">API endpoint URL</label>
											<input type="text" class="zo-asp-input zo-asp-api-url" placeholder="Auto-filled when you change provider" />
										</div>
										...
										<div>
											...
											<input type="text" class="zo-asp-input zo-asp-api-model" value="gemini-image" />
										</div>

										<div class="zo-asp-note">
											Gemini, HuggingFace, fal.ai, SD, and self-hosted are all wired in. Browser mode exposes keys to the client, so this is best for testing unless you proxy requests through your own server.
										</div>
									</div>
								</div>
							...
							<div class="zo-asp-stack">
								...
								<div class="zo-asp-card">
									...
									<ul class="zo-asp-list">
										...
										<li>Real AI API mode supports Gemini, HuggingFace, fal.ai, Stable Diffusion / FLUX style endpoints, and self-hosted APIs.</li>
								...
								<div class="zo-asp-card">
									...
									<ul class="zo-asp-list">
										...
										<li>Switch between all 5 API providers and verify each response format.</li>
									</ul>
								</div>
							</div>
						</div>
...
return array(
	...
	'author'          => 'Asker',
	'description'     => 'Bilingual AI image studio with optional real API mode, 5 provider support, auto translation, coins, kid-safe mode, and share links.',
